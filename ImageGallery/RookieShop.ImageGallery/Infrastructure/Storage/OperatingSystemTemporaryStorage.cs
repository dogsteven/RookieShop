...
{
    public Task<Stream> OpenAsStreamAsync(string path, CancellationToken cancellationToken)
    {
        return Task.FromResult<Stream>(new FileStream(path, FileMode.Open));
    }

    public async Task<string> SaveStreamAsync(Stream stream, CancellationToken cancellationToken)
    {
